...
- 必須に指定された環境変数を指定しなかった場合は起動に失敗します。
- 必須ではない環境変数が指定しなかった場合はデフォルト値が使用されます。

| 変数名            | 説明                                                                                                                                                                      | デフォルト値 | 必須  |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ | ----- |
| `DISCORD_TOKEN`   | BOT のトークン                                                                                                                                                            | なし         | True  |
| `MAIN_CHANNEL_ID` | VoiceDiff(VC 入退室ログ)を送信する **テキスト** チャンネルの ID                                                                                                           | なし         | True  |
| `GUILD_ID`        | 限界開発鯖の ID                                                                                                                                                           | なし         | True  |
| `PREFIX`          | コマンドの接頭辞、デフォルト値は `"!"`                                                                                                                                    | `!`          | False |
| `FEATURE`         | 有効にする機能のカンマ区切り文字列、デフォルト値は全ての機能。`"MESSAGE_CREATE"`, `"MESSAGE_UPDATE"`, `"COMMAND"`, `"VOICE_ROOM"`, `"ROLE"`, `"EMOJI"` を組み合わせ可能。 | 全ての機能   | False |
